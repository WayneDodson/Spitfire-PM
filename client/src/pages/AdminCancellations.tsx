...
const REASON_COLORS: Record<string, string> = {
  ...
  other: "bg-white/5 text-foreground/50 border-border",
};
    ...
    return (
      <div className="min-h-screen bg-[#060d1a] flex items-center justify-center px-4">
        <div className="text-center">
          ...
          <p className="text-foreground/50 mb-6">This page is restricted to admin users.</p>
        ...
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-foreground/40 text-xs">Understanding why users leave — and how to support them</p>
            </div>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-muted/60 border border-border rounded-xl p-4">
            <p className="text-foreground/40 text-xs uppercase tracking-widest mb-1">Total Cancellations</p>
        ...
        {Object.keys(reasonCounts).length > 0 && (
          <div className="bg-muted/50 border border-border rounded-xl p-5 mb-8">
            <h3 className="text-sm font-semibold text-foreground/60 uppercase tracking-widest mb-4">Reason Breakdown</h3>
            <div className="flex flex-wrap gap-2">
              {Object.entries(reasonCounts)
                ...
                .map(([reason, count]) => (
                  <div
                    key={reason}
                    className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-xs font-medium ${REASON_COLORS[reason] ?? "bg-white/5 text-foreground/50 border-border"}`}
                  >
        ...
        {/* Tabs */}
        <div className="flex gap-1 bg-muted/60 border border-border rounded-xl p-1 mb-6 w-fit">
          {tabs.map((tab) => {
            const Icon = tab.icon;
            return (
              <button
        ...
        {activeTab === "reasons" && (
          <div className="space-y-3">
            {loadingReasons ? (
              ...
              reasons?.map((r) => (
                <div key={r.id} className="bg-muted/50 border border-border rounded-xl p-5">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mb-2">
                        <User className="h-3.5 w-3.5 text-foreground/30 flex-shrink-0" />
                        <span className="text-sm text-foreground/70">{r.userName || "Unknown"}</span>
                        {r.userEmail && <span className="text-xs text-foreground/30">({r.userEmail})</span>}
                      </div>
                      {(r.currentIndustry || r.targetRole) && (
                        <div className="flex items-center gap-4 text-xs text-foreground/40 mb-2">
                          {r.currentIndustry && <span>From: {r.currentIndustry}</span>}
                          {r.targetRole && <span>→ {r.targetRole}</span>}
                        </div>
                      )}
                      {r.customReason && (
                        <p className="text-sm text-foreground/50 italic mt-2">"{r.customReason}"</p>
                      )}
                    </div>
                    <div className="flex flex-col items-end gap-2 text-xs text-foreground/30 flex-shrink-0">
          ...
          <div className="space-y-4">
            {loadingMentor ? (
              ...
              mentorRequests?.map((req) => (
                <div key={req.id} className="bg-muted/50 border border-border rounded-xl p-5">
                  <div className="flex items-start justify-between gap-4 mb-4">
                    <div>
                      <div className="flex items-center gap-3 mb-1">
                        <span className={`px-2.5 py-1 rounded-full border text-xs font-medium ${
                          req.status === "pending"
                            ...
                            : "bg-white/5 text-foreground/40 border-border"
                        }`}>
                          {req.status}
                        </span>
                        <span className="text-foreground/30 text-xs">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-foreground/70">{req.userName || "Unknown"}</span>
                        {req.userEmail && <span className="text-xs text-foreground/30">({req.userEmail})</span>}
                      </div>
                  ...
                  {req.helpTopics && (
                    <div className="mb-3">
                      <p className="text-xs text-foreground/30 uppercase tracking-widest mb-2">Help topics</p>
                  ...
                  <div className="bg-muted/60 border border-border rounded-lg p-4 mb-3">
                    <div className="flex items-start gap-2">
                      <MessageSquare className="h-4 w-4 text-cyan-400 flex-shrink-0 mt-0.5" />
                      <div>
                        <p className="text-xs text-foreground/30 uppercase tracking-widest mb-1">Main question</p>
                        <p className="text-sm text-foreground/80 leading-relaxed">"{req.mainQuestion}"</p>
                      </div>
                    </div>
                  </div>
                  ...
                  {req.currentSituation && (
                    <div className="mb-3">
                      <p className="text-xs text-foreground/30 uppercase tracking-widest mb-1">Current situation</p>
                      <p className="text-sm text-foreground/60 leading-relaxed">{req.currentSituation}</p>
                    </div>
                  )}

                  {req.desiredOutcome && (
                    <div>
                      <p className="text-xs text-foreground/30 uppercase tracking-widest mb-1">Desired outcome</p>
                      <p className="text-sm text-foreground/60 leading-relaxed">{req.desiredOutcome}</p>
                    </div>
                  )}
                </div>
              ))
                ...
                <div key={opt.id} className={`border rounded-xl p-5 ${
                  ...
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-sm font-semibold text-foreground/80">{opt.userName || "Unknown"}</span>
                          {opt.userEmail && <span className="text-xs text-foreground/30">({opt.userEmail})</span>}
                        </div>
                    ...
                    <div className="text-right text-xs text-foreground/30">
