...
// FIX: What's included in Level 1 free trial
const FREE_TRIAL_INCLUDES = [
  { icon: BookOpen, label: "6 structured lessons", detail: "Foundation PM concepts, bite-sized and digestible" },
  { icon: Brain, label: "Knowledge checks", detail: "Quiz after each lesson — prove you understood it" },
  { icon: PlayCircle, label: "2 live simulations", detail: "Real PM scenarios with consequence-based decisions" },
  { icon: Target, label: "Confidence Score", detail: "Track your readiness from day one" },
  { icon: BarChart3, label: "Interview readiness", detail: "See exactly where you stand before you subscribe" },
  { icon: Zap, label: "No credit card needed", detail: "Start free, upgrade only when you're ready" },
];

// FIX: Social proof quotes (ready for real quotes)
const TESTIMONIALS = [
  {
...
export default function Home() {
  ...
  // FIX: Scroll to how-it-works section
  ...
  return (
    <div className="min-h-screen bg-[#080c14] text-white overflow-x-hidden">
      {/* FIX: Brand name is now consistently "Spitfire PM" throughout */}
      <nav className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-[#080c14]/90 backdrop-blur-md">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            {/* FIX: Was "PM Simulate" — now matches domain & footer */}
            <span className="font-bold text-lg tracking-tight">Spitfire PM</span>
      ...
      <section className="pt-32 pb-24 px-6 relative">
        ...
        <div className="max-w-5xl mx-auto text-center relative">
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center mb-12">
            ...
            {/* FIX: Was linking to /login — now scrolls to how-it-works section */}
            <Button
              size="lg"
              variant="outline"
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-white/60 text-lg leading-relaxed">
                Spitfire PM bridges that gap. Real scenarios. Real decisions. Real confidence built
      ...
      {/* FIX: NEW — What's included in Level 1 free */}
      <section className="py-24 px-6 bg-gradient-to-b from-cyan-950/10 to-transparent border-y border-cyan-400/10">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <h2 className="text-4xl md:text-5xl font-black mb-4">
              Start today. No card. No risk.
            </h2>
            <p className="text-white/50 text-lg max-w-2xl mx-auto">
              Your free access includes everything you need to decide if Spitfire PM is right for you —
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-10">
            {FREE_TRIAL_INCLUDES.map(({ icon: Icon, label, detail }) => (
              <div
                key={label}
                className="bg-white/[0.03] border border-white/10 rounded-xl p-5 flex gap-4 items-start hover:border-cyan-400/20 transition-colors"
              >
                <div className="w-9 h-9 bg-cyan-400/10 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
          ...
          <div className="text-center">
            ...
            <p className="text-white/30 text-sm mt-4">No credit card · Cancel anytime · Upgrade when ready</p>
          </div>
        </div>
      </section>

      {/* FIX: id added so "See How It Works" scrolls here */}
      ...
      {/* FIX: NEW — Social proof / Testimonials */}
        ...
        <div className="max-w-5xl mx-auto">
          ...
          <div className="grid sm:grid-cols-3 gap-5">
            {TESTIMONIALS.map(({ quote, name, role }) => (
              <div
                key={name}
                className="bg-[#0d1420] border border-white/10 rounded-2xl p-6 flex flex-col"
              >
                ...
                <p className="text-white/70 text-sm leading-relaxed mb-6 flex-1">&ldquo;{quote}&rdquo;</p>
                <div>
      ...
      <section className="py-24 px-6 bg-white/[0.02] border-y border-white/5">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-14">
            ...
            <p className="text-white/50 text-lg max-w-2xl mx-auto">
              Level 1 is always free. Full platform access from £39/month — or save with annual.
            </p>
          </div>
          <div className="grid sm:grid-cols-3 gap-5 max-w-4xl mx-auto">
            {/* Free */}
            <div className="bg-[#0d1420] border border-white/10 rounded-2xl p-6">
              <p className="text-white/40 text-sm font-medium uppercase tracking-widest mb-2">Free</p>
              <p className="text-4xl font-black mb-1">£0</p>
              <p className="text-white/40 text-sm mb-6">Level 1 — always free</p>
            ...
            {/* Professional */}
            <div className="bg-[#0d1420] border-2 border-cyan-400/40 rounded-2xl p-6 relative">
              <div className="absolute -top-3 left-1/2 -translate-x-1/2 bg-cyan-500 text-black text-xs font-bold px-3 py-1 rounded-full">
                Most Popular
              </div>
              <p className="text-cyan-400 text-sm font-medium uppercase tracking-widest mb-2">Professional</p>
              <p className="text-4xl font-black mb-1">£39<span className="text-xl text-white/40 font-normal">/mo</span></p>
              <p className="text-white/40 text-sm mb-6">Cancel anytime</p>
              <ul className="space-y-2 mb-6">
                {["All 7 levels", "All simulations", "AI feedback", "PM Readiness Certificate", "Interview toolkit"].map((f) => (
                  <li key={f} className="flex items-center gap-2 text-sm text-white/60">
                    <CheckCircle2 className="h-3.5 w-3.5 text-cyan-400 flex-shrink-0" />
                    {f}
                  </li>
            ...
            {/* Annual */}
            <div className="bg-[#0d1420] border border-white/10 rounded-2xl p-6">
              <p className="text-white/40 text-sm font-medium uppercase tracking-widest mb-2">Annual</p>
              <div className="flex items-baseline gap-2 mb-1">
                <p className="text-4xl font-black">£197</p>
                <span className="text-xs text-green-400 bg-green-400/10 border border-green-400/20 rounded-full px-2 py-0.5">Save £271</span>
              </div>
              <p className="text-white/40 text-sm mb-6">£16.42/month — best value</p>
              <ul className="space-y-2 mb-6">
                {["Everything in Professional", "Lowest monthly cost", "Founder access pricing", "12 months full access"].map((f) => (
                  <li key={f} className="flex items-center gap-2 text-sm text-white/60">
                    <CheckCircle2 className="h-3.5 w-3.5 text-cyan-400 flex-shrink-0" />
                    {f}
                  </li>
          ...
          <p className="text-center text-white/30 text-sm mt-8">
            Consistent trial users earn Founder Access at £19/month for the first 6 months — commitment is rewarded.
          </p>
        </div>
      </section>
      ...
      {/* Footer — FIX: Brand name consistent with nav */}
